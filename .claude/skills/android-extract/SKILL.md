...
1. **sp 値（フォントサイズ）→ `MaterialTheme.typography.*`**  
   `WeatherNowTypography` で定義済みのスタイルに対応する。sp のハードコードは `Text` の `style` 引数ごと Token に置き換える：
   - `fontSize = 57.sp` → `style = MaterialTheme.typography.displayLarge`
   - `fontSize = 14.sp` → `style = MaterialTheme.typography.bodyMedium`  
   使用しているサイズと `WeatherNowTypography` の対応（xs=11, sm=12, md=14, lg=16, xl=22, 2xl=45, 3xl=57）を照合して判断する。
2. **dp 値（余白・サイズ）→ Kotlin Token を優先する**
...
3. **dimens.xml は例外のみ** — 端末サイズ修飾子（`-sw600dp` 等）が必要な値だけ `dimens.xml` に追加する

**除外**:
- `0.dp`, `1.dp`（ゼロや境界線はそのまま）
- すでに Token クラスまたは `MaterialTheme.typography.*` を使っている箇所
...
**原則: sp はタイポグラフィ Token、dp は Spacing/Size Token を使う（dimens.xml は例外）**

#### sp 値（フォントサイズ）→ `WeatherNowTypography` 経由で `MaterialTheme.typography.*` へ

| `MaterialTheme.typography.*` | fontSize | 用途 |
|---|---|---|
| `.displayLarge` | 57sp | 気温などの主要数値 |
| `.displayMedium` | 45sp | エラー絵文字 |
| `.titleLarge` | 22sp | 天気コード説明文 |
| `.titleMedium` | 16sp | セクション見出し |
| `.bodyLarge` | 16sp | エラーメッセージ本文 |
| `.bodyMedium` | 14sp | 曜日・気温範囲 |
| `.bodySmall` | 12sp | 予報の補足 |
| `.labelSmall` | 11sp | 湿度・風速のラベル |

#### dp 値（余白・サイズ）→ `WeatherNowSpacing` / `WeatherNowSize` へ
...
### B. dp / sp 値の書き換え（Kotlin Token）

**sp 値 → `MaterialTheme.typography.*`**

```kotlin
// Before: Text(text = "...", fontSize = 14.sp)
// After:  Text(text = "...", style = MaterialTheme.typography.bodyMedium)

// Before: Text(text = "...", fontSize = 57.sp, fontWeight = FontWeight.Light)
// After:  Text(text = "...", style = MaterialTheme.typography.displayLarge)
```

sp のハードコードは `fontSize` プロパティだけでなく、**`style` 引数全体をまとめて Token に置き換える**。
個別の `fontSize`/`fontWeight` 指定は残さない。

**dp 値 → `WeatherNowSpacing` / `WeatherNowSize`**
...
- **sp 値は `MaterialTheme.typography.*`（WeatherNowTypography）が第一選択**。`fontSize` ハードコードは残さない
